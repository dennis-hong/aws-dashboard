...
from aws_dashboard.content.aws.access_and_security.keypairs \
    import forms as aws_forms

from aws_dashboard.api import ec2


class CreateView(forms.ModalFormView):
    form_class = aws_forms.CreateKeypair
    form_id = "create_keypair_form"
    modal_header = _("Create Key Pair")
    template_name = 'aws/access_and_security/keypairs/create.html'
    submit_label = _("Create Key Pair")
    submit_url = reverse_lazy(
        "horizon:aws:access_and_security:keypairs:create")
    success_url = 'horizon:aws:access_and_security:keypairs:download'
    page_title = _("Create Key Pair")
    cancel_url = reverse_lazy(
        "horizon:aws:access_and_security:index")
...
class ImportView(forms.ModalFormView):
    form_class = aws_forms.ImportKeypair
    form_id = "import_keypair_form"
    modal_header = _("Import Key Pair")
    template_name = 'aws/access_and_security/keypairs/import.html'
    submit_label = _("Import Key Pair")
    submit_url = reverse_lazy(
        "horizon:aws:access_and_security:keypairs:import")
...
class DetailView(views.HorizonTemplateView):
    template_name = 'aws/access_and_security/keypairs/detail.html'
    page_title = _("Key Pair Details")

    @memoized.memoized_method
    def _get_data(self):
        try:
            keypair = ec2.get_keypair(self.request,
...
class DownloadView(views.HorizonTemplateView):
    template_name = 'aws/access_and_security/keypairs/download.html'
    ...
    def get(self, request, keypair_name=None, optional=None):
        try:
            if optional == "regenerate":
                ec2.delete_keypair(request, keypair_name)

            keypair = ec2.create_keypair(request, keypair_name)
        except Exception:
            redirect = reverse('horizon:aws:access_and_security:index')
