...
class ImportGroup(forms.SelfHandlingForm):
    security_group = forms.ChoiceField(label=_("OpenStack Security Group"))
    ...
    def handle(self, request, data):
        ...
        try:
            ec2.import_openstack_sg(request, security_group)
            msg = _('Import security group from OpenStack %s.') % security_group
            messages.success(request, msg)
        except Exception:
            redirect = reverse('horizon:aws:access_and_security:index')
            exceptions.handle(request, _("Unable to import security group."),
                              redirect=redirect)
        return True
...
class UpdateGroup(GroupBase):
    ...
    def _call_network_api(self, request, data):
        return ec2.update_security_group(request,
                                         data['id'],
                                         data['name'],
                                         data['description'])
...
class AddRule(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        redirect = reverse("aws:ec2:access_and_security:"
                           "security_groups:detail", args=[data['id']])
        try:
            rule = ec2.create_security_group_rule(
                request,
