...
(function () {
  ...
  angular
    ...
    .controller('ExportInstanceFlavorController', ExportInstanceFlavorController);

  ExportInstanceFlavorController.$inject = [
    '$scope',
    'horizon.framework.widgets.charts.quotaChartDefaults',
    'exportInstanceModel'
  ];

  function ExportInstanceFlavorController($scope, quotaChartDefaults, exportInstanceModel) {
    var ctrl = this;
    ...
    // Convenience references to export instance model elements
    ctrl.flavors = [];
    ctrl.metadataDefs = exportInstanceModel.metadataDefs;
    ctrl.novaLimits = {};
    ...
    var novaLimitsWatcher = $scope.$watch(function () {
      return exportInstanceModel.novaLimits;
    ...
    var flavorsWatcher = $scope.$watchCollection(function() {
      return exportInstanceModel.flavors;
    }, function (newValue, oldValue, scope) {
      var ctrl = scope.selectFlavorCtrl;
    ...
    var instanceCountWatcher = $scope.$watch(function () {
      return exportInstanceModel.newInstanceSpec.instance_count;
    }, function (newValue, oldValue, scope) {
    ...
    var facadesWatcher = $scope.$watchCollection(
      ...
      function (newValue, oldValue, scope) {
        if (newValue && newValue.length > 0) {
          exportInstanceModel.newInstanceSpec.flavor = newValue[0].flavor;
          scope.selectFlavorCtrl.validateFlavor();
        } else {
          delete exportInstanceModel.newInstanceSpec.flavor;
        }
      }
    );

    var sourceWatcher = $scope.$watchCollection(function() {
      return exportInstanceModel.newInstanceSpec.source;
    }, function (newValue, oldValue, scope) {
      var ctrl = scope.selectFlavorCtrl;
    ...
    function validateFlavor() {
      ...
      if (allocatedFlavors && allocatedFlavors.length > 0) {
        ...
        $scope.exportInstanceFlavorForm['allocated-flavor']
    ...
    function updateFlavorFacades() {
      ...
      var instancesChartData = ctrl.getChartData(
        ...
        ctrl.instanceCount,
        exportInstanceModel.novaLimits.totalInstancesUsed,
        exportInstanceModel.novaLimits.maxTotalInstances);
      ...
      for (var i = 0; i < ctrl.availableFlavorFacades.length; i++) {
        ...
        facade.vcpusChartData = ctrl.getChartData(
          ...
          exportInstanceModel.novaLimits.totalCoresUsed,
          exportInstanceModel.novaLimits.maxTotalCores);

        facade.ramChartData = ctrl.getChartData(
          ctrl.chartTotalRamLabel,
          ctrl.instanceCount * facade.ram,
          exportInstanceModel.novaLimits.totalRAMUsed,
          exportInstanceModel.novaLimits.maxTotalRAMSize);
    ...
    function getErrors(flavor) {
      ...
      var sourceType = exportInstanceModel.newInstanceSpec.source_type;
