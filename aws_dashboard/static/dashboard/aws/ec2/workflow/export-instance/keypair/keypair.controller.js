...
(function () {
  ...
  angular
    ...
    .controller('ExportInstanceKeypairController', ExportInstanceKeypairController);

  ExportInstanceKeypairController.$inject = [
  ...
  /**
   * @ngdoc controller
   * @name ExportInstanceKeypairController
   ...
   */
  function ExportInstanceKeypairController(
    basePath,
    exportInstanceModel,
    $modal,
    toastService,
    ...
    ctrl.tableData = {
      available: exportInstanceModel.keypairs,
      allocated: exportInstanceModel.newInstanceSpec.key_pair
    };
     ...
     * @name createKeyPair
     * @description
     * Exportes the modal to create a key pair.
     * @returns {undefined} No return value
     */
    function createKeyPair() {
      $modal.open({
        templateUrl: basePath + 'keypair/create-keypair.html',
        controller: 'ExportInstanceCreateKeyPairController as ctrl',
     ...
     * @name importKeyPair
     * @description
     * Exportes the modal to import a key pair.
     * @returns {undefined} No return value
     */
    function importKeyPair() {
      $modal.open({
        templateUrl: basePath + 'keypair/import-keypair.html',
        controller: 'ExportInstanceImportKeyPairController as ctrl',
        windowClass: 'modal-dialog-wizard'
    ...
    function assignKeypair(keypair) {
      ...
      exportInstanceModel.keypairs.push(keypair);
      ctrl.allocateNewKeyPair(keypair);
    }

    function getKeypairs() {
      return exportInstanceModel.keypairs.map(getName);
    }
