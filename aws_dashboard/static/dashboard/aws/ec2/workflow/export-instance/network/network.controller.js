...
(function () {
  ...
  /**
   * @ngdoc controller
   * @name ExportInstanceNetworkController
   * @description
   * Controller for the Export Instance - Network Step.
   */
  angular
    .module('horizon.dashboard.aws.workflow.export-instance')
    .controller('ExportInstanceNetworkController', ExportInstanceNetworkController);

  ExportInstanceNetworkController.$inject = [
    '$scope',
    'horizon.framework.widgets.action-list.button-tooltip.row-warning.service',
    'exportInstanceModel'
  ];

  function ExportInstanceNetworkController($scope, tooltipService, exportInstanceModel) {
    var ctrl = this;
    ...
    function getPorts() {
      return exportInstanceModel.newInstanceSpec.ports;
    }
