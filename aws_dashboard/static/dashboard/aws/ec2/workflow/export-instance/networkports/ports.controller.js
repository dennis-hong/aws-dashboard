...
(function () {
  ...
  /**
   * @ngdoc controller
   * @name ExportInstanceNetworkPortController
   * @description
   * Controller for the Export Instance - Network Step.
   */
  angular
    .module('horizon.dashboard.aws.workflow.export-instance')
    .controller('ExportInstanceNetworkPortController', ExportInstanceNetworkPortController);

  ExportInstanceNetworkPortController.$inject = [
    '$scope',
    'horizon.framework.widgets.action-list.button-tooltip.row-warning.service'
  ];

  function ExportInstanceNetworkPortController($scope, tooltipService) {
    var ctrl = this;
