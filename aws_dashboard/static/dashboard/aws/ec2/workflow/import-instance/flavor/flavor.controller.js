...
(function () {
  ...
  angular
    ...
    .controller('ImportEC2InstanceFlavorController', ImportEC2InstanceFlavorController);

  ImportEC2InstanceFlavorController.$inject = [
    '$scope',
    'horizon.framework.widgets.charts.quotaChartDefaults',
    'importEC2InstanceModel'
  ];

  function ImportEC2InstanceFlavorController($scope, quotaChartDefaults, importEC2InstanceModel) {
    var ctrl = this;
    ...
    // Convenience references to import instance model elements
    ctrl.flavors = [];
    ctrl.metadataDefs = importEC2InstanceModel.metadataDefs;
    ctrl.novaLimits = {};
    ...
    var novaLimitsWatcher = $scope.$watch(function () {
      return importEC2InstanceModel.novaLimits;
    ...
    var flavorsWatcher = $scope.$watchCollection(function() {
      return importEC2InstanceModel.flavors;
    }, function (newValue, oldValue, scope) {
      var ctrl = scope.selectFlavorCtrl;
    ...
    var instanceCountWatcher = $scope.$watch(function () {
      return importEC2InstanceModel.newInstanceSpec.instance_count;
    }, function (newValue, oldValue, scope) {
    ...
    var facadesWatcher = $scope.$watchCollection(
      ...
      function (newValue, oldValue, scope) {
        if (newValue && newValue.length > 0) {
          importEC2InstanceModel.newInstanceSpec.flavor = newValue[0].flavor;
          scope.selectFlavorCtrl.validateFlavor();
        } else {
          delete importEC2InstanceModel.newInstanceSpec.flavor;
        }
      }
    );

    var sourceWatcher = $scope.$watchCollection(function() {
      return importEC2InstanceModel.newInstanceSpec.source;
    }, function (newValue, oldValue, scope) {
      var ctrl = scope.selectFlavorCtrl;
    ...
    function validateFlavor() {
      ...
      if (allocatedFlavors && allocatedFlavors.length > 0) {
        ...
        $scope.importInstanceFlavorForm['allocated-flavor']
    ...
    function updateFlavorFacades() {
      ...
      var instancesChartData = ctrl.getChartData(
        ...
        ctrl.instanceCount,
        importEC2InstanceModel.novaLimits.totalInstancesUsed,
        importEC2InstanceModel.novaLimits.maxTotalInstances);
      ...
      for (var i = 0; i < ctrl.availableFlavorFacades.length; i++) {
        ...
        facade.vcpusChartData = ctrl.getChartData(
          ...
          importEC2InstanceModel.novaLimits.totalCoresUsed,
          importEC2InstanceModel.novaLimits.maxTotalCores);

        facade.ramChartData = ctrl.getChartData(
          ctrl.chartTotalRamLabel,
          ctrl.instanceCount * facade.ram,
          importEC2InstanceModel.novaLimits.totalRAMUsed,
          importEC2InstanceModel.novaLimits.maxTotalRAMSize);
    ...
    function getErrors(flavor) {
      ...
      var sourceType = importEC2InstanceModel.newInstanceSpec.source_type;
